...
const amazon_sdk_1 = require("../../utils/amazon-sdk");
...
let HotelRepo = class HotelRepo {
    ...
    async fillDatesTandC(HotelRateByDate) {
    ...
    async emailBookingSend(template, to, subject, BookingConfirmation, CreateDate, HotelPropertyName, CheckInDate, CheckOutDate, Bookinglink) {
        return new Promise(async (resolve) => {
    ...
    async http_currenciesBySymbols(base) {
        return new Promise(async (resolve) => {
            var config = {
                method: 'get',
    ...
    async convertPriceToBase(base, Price) {
        return new Promise(async (resolve) => {
            let CACHE_TIME = parseInt(process.env.Cache_Time);
    ...
    async http_HotelDetailsAvailability(data) {
        ...
        return new Promise(async (resolve) => {
            try {
                let xmlData = fs.readFileSync(path.resolve(__dirname, './soapXML/getHotelDetail.xml'), 'utf-8');
                ...
                if (response.status == 200) {
                    ...
                    parseString(response.data, function (err, result) {
                        ...
                        if (requestResult['SOAP:Fault'] || requestResult['SOAP-ENV:Fault']) {
                            ...
                            let responseData = { "status": status, "faultcode": faultcode, "message": message };
    ...
    async multiSelectHotelDetails(data, requestFrom) {
        return new Promise(async (resolve) => {
            let AdultsAndRooms = await this.minRequestforHotelDetailsUpdate(data.AdultsAndRooms, data.ParticipationLevel);
            ...
            if (data_rLength >= 1) {
                resolve(ResponseBuilder_1.ResponseBuilder.successResponse('Data', data_r[0]));
            }
            else {
                resolve(ResponseBuilder_1.ResponseBuilder.errorResponse('Data', hotelDetails));
            }
            console.log(data_r);
        });
    }
    async hotelDetails(data, modifyData) {
        return new Promise(async (resolve) => {
            await this.mongodb.connect();
            let CACHE_TIME = parseInt(process.env.Cache_Time);
            let findCacheDocument = await this.mongodb.findCacheHotelDetailsDocument({ "hash": { $eq: modifyData.hash } });
            let requestResults_hotel_Json = [];
            let requestResults_images_Json = [];
            if (findCacheDocument.length == 0) {
                ...
                if (hotelDetails.HotelAlternateProperties == true) {
                    resolve(ResponseBuilder_1.ResponseBuilder.successResponse('Data', hotelDetails));
                }
                else {
                    let TargetBranch = data.TraceId;
                    let HotelChain = data.HotelChain;
                    ...
                    if (hotelDetails.status == 404) {
                        resolve(ResponseBuilder_1.ResponseBuilder.errorMultiResponse('message', hotelDetails['message'], 'faultcode', hotelDetails['faultcode']));
                    }
                    else {
                        ...
                        resolve(ResponseBuilder_1.ResponseBuilder.successResponse('Data', modifiedWithImagesData[0]));
                    }
                }
            }
            else {
                ...
                if (timeAgo >= CACHE_TIME) {
                    ...
                    if (httpData.status == 404) {
                        resolve(ResponseBuilder_1.ResponseBuilder.errorMultiResponse('message', httpData['message'], 'faultcode', httpData['faultcode']));
                    }
                    else {
                        if (httpData.HotelAlternateProperties == true) {
                            resolve(ResponseBuilder_1.ResponseBuilder.successResponse('Data', httpData));
                        }
                        else {
                            let requestResults_hotel_Json = [];
                            let requestResults_images_Json = [];
    ...
    async http_moreHotelSearchingAvailability(data, modifyData) {
        let node_env = process.env.NODE_ENV;
        let TARGET_BRANCH;
        let PROVIDER_CODE;
        let ACCEPT_ENCODING;
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (modifyData.mode == "latlng") {
                    let xmlData = fs.readFileSync(path.resolve(__dirname, './soapXML/moreHotelSearchLatLog.xml'), 'utf-8');
                    let _replace_data = xmlData.split('--TraceId--').join(dataValues.TraceId);
                    ...
                    let xmlData = fs.readFileSync(path.resolve(__dirname, './soapXML/hotelSearch.xml'), 'utf-8');
                    let _replace_data = xmlData.split('--TraceId--').join(dataValues.TraceId);
                    _replace_data = _replace_data.split('--HotelLoc--').join(dataValues.HotelLoc);
                    _replace_data = _replace_data.split('--ReferencePoint--').join(dataValues.ReferencePoint);
                    _replace_data = _replace_data.split('--Adults--').join(dataValues.Adults);
    ...
    async http_HotelSearchingAvailability(data, modifyData) {
        let node_env = process.env.NODE_ENV;
        let TARGET_BRANCH;
        let PROVIDER_CODE;
        let ACCEPT_ENCODING;
        ...
        return new Promise(async (resolve) => {
            try {
                let _build_data;
                if (modifyData.mode == "latlng") {
                    let xmlData = fs.readFileSync(path.resolve(__dirname, './soapXML/hotelSearchLatLog.xml'), 'utf-8');
                    let _replace_data = xmlData.split('--TraceId--').join(dataValues.TraceId);
                    _replace_data = _replace_data.split('--latitude--').join(dataValues.Latitude);
                    _replace_data = _replace_data.split('--Base--').join(dataValues.Base);
                    _replace_data = _replace_data.split('--AvailableHotelsOnly--').join(dataValues.AvailableHotelsOnly);
    ...
    async moreHotelServiceAvailabilityRequest(data, modifyData) {
        return new Promise(async (resolve) => {
            await this.mongodb.connect();
            let CACHE_TIME = parseInt(process.env.Cache_Time);
            ...
            if (findCacheDocument.length == 0) {
                ...
                if (httpStatus == 404) {
                    resolve(ResponseBuilder_1.ResponseBuilder.errorMultiResponse('message', hotelList['message'], 'faultcode', hotelList['faultcode']));
                }
                else {
                    ...
                    resolve(ResponseBuilder_1.ResponseBuilder.successResponse('Data', data_r));
                }
            }
            else {
                let timeAgo = await this.timeAgo(findCacheDocument[0].timeout);
    ...
    async hotelServiceAvailabilityRequest(data, modifyData) {
        return new Promise(async (resolve) => {
            await this.mongodb.connect();
            let CACHE_TIME = parseInt(process.env.Cache_Time);
            let findCacheDocument = await this.mongodb.findCacheDocument({ "hash": { $eq: modifyData.hash } });
            ...
            if (findCacheDocument.length == 0) {
                ...
                let httpStatus = hotelList['status'];
                if (httpStatus == 404) {
                    resolve(ResponseBuilder_1.ResponseBuilder.errorMultiResponse('message', httpStatus['message'], 'faultcode', httpStatus['faultcode']));
                }
                else {
                    for (let i = 0; i < hotelList['Hotels'].length; i++) {
                        let findHotelDocument = await this.mongodb.findHotelDocument({ "HotelId": { $eq: hotelList['Hotels'][i].HotelId } });
    ...
    async http_GetMediaLinks(data) {
        ...
        return new Promise(async (resolve) => {
            try {
                let xmlData = fs.readFileSync(path.resolve(__dirname, './soapXML/getMediaLinks.xml'), 'utf8');
                let _replace_data = xmlData.split('--TraceId--').join(dataValues.TraceId);
                _replace_data = _replace_data.split('--HotelChain--').join(dataValues.HotelChain);
                _replace_data = _replace_data.split('--TargetBranch--').join(TARGET_BRANCH);
    ...
    async hotelMediaLinks(data, modifyData) {
        return new Promise(async (resolve) => {
            ...
            if (findMediaLinkDocument.length == 0) {
                ...
                if (httpStatus == 404) {
                    resolve(ResponseBuilder_1.ResponseBuilder.errorMultiResponse('message', reData['message'], 'faultcode', reData['faultcode']));
                }
                else {
                    ...
                    resolve(ResponseBuilder_1.ResponseBuilder.successResponse('Data', databaseData[0]));
                }
            }
            else {
    ...
    async http_HotelTermsAndCondition(data) {
        let node_env = process.env.NODE_ENV;
        let TARGET_BRANCH;
        let PROVIDER_CODE;
        let ACCEPT_ENCODING;
        ...
        return new Promise(async (resolve) => {
            try {
                let xmlData = fs.readFileSync(path.resolve(__dirname, './soapXML/hotelTermsAndCondition.xml'), 'utf-8');
        ...
        return new Promise(async (resolve) => {
            ...
            if (findHotelTNCCacheDocument.length == 0) {
                ...
                if (httpStatus == 404) {
                    ...
                    resolve(ResponseBuilder_1.ResponseBuilder.successResponse('Data', databaseData[indexLevel]));
                }
            }
            else {
    ...
    async testing(body) {
        return new Promise(async (resolve) => {
            try {
                let requestResults_hotel = fs.readFileSync(path.resolve(__dirname, './soapXML/hotel-d.xml'), 'utf-8');
                let requestResults_images = fs.readFileSync(path.resolve(__dirname, './soapXML/images-d.xml'), 'utf-8');
                let requestResults_hotel_Json = [];
                let requestResults_images_Json = [];
                parseString(requestResults_images, async function (err, result) {
                    ...
                    if (requestResult['SOAP:Fault'] || requestResult['SOAP-ENV:Fault']) {
                        ...
                        let message = requestResult['SOAP:Fault'][0]['faultstring'][0];
                        let faultcode = message.split(" ");
                        let responseData = { "status": status, "faultcode": faultcode[0], "message": message };
                        ...
                        try {
                            ...
                            let status = 404;
                            let responseData = { "status": status, "message": 'Invalid Hotel Code' };
                            requestResults_images_Json.push(responseData);
                        }
                    }
                });
    ...
    async cityAutoComplete(data) {
        return new Promise(async (resolve) => {
            ...
            if (iataLength == 0) {
                resolve(ResponseBuilder_1.ResponseBuilder.errorResponse('message', 'No Data Found'));
            }
            else {
                resolve(ResponseBuilder_1.ResponseBuilder.successResponse('Data', iata));
            }
        });
    }
    async HotelRateDetailForFilters(hotelRateDetail) {
    ...
    async popularDestinations(body, ip) {
        return new Promise(async (resolve) => {
            console.log(ip);
            let ip_data = await axios_1.default.get('http://ip-api.com/json/' + ip).then(function (response) {
    ...
    async http_GetMultiMediaLinks(body) {
        let node_env = process.env.NODE_ENV;
        let TARGET_BRANCH;
        let PROVIDER_CODE;
        let ACCEPT_ENCODING;
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let xmlData = fs.readFileSync(path.resolve(__dirname, './soapXML/getMultiMediaLinks.xml'), 'utf-8');
                    ...
                    parseString(response.data, async function (err, result) {
                        ...
                        if (requestResult['SOAP:Fault'] || requestResult['SOAP-ENV:Fault']) {
                            ...
                            try {
                                ...
                                resolve(ResponseBuilder_1.ResponseBuilder.successResponse('Data', mediaData));
                            }
                            catch (e) {
                                status = 404;
                                responseData = { "status": status, "message": 'Invalid Hotel Code' };
                                resolve(ResponseBuilder_1.ResponseBuilder.errorResponse('message', responseData));
                            }
                        }
                    });
                }
    ...
    async http_Booking(body) {
        ...
        return new Promise(async (resolve) => {
            try {
                let xmlData = fs.readFileSync(path.resolve(__dirname, './soapXML/booking.xml'), 'utf-8');
                let _replace_data = xmlData.split('--TraceId--').join(body.TraceId);
    ...
    async userBooking(body) {
        return new Promise(async (resolve) => {
            ...
            if (BookingAsGuest == true) {
                let http_Booking = await this.http_Booking(body);
                let httpStatus = http_Booking['status'];
                if (httpStatus == 404) {
                    resolve(ResponseBuilder_1.ResponseBuilder.errorMultiResponse('message', http_Booking['message'], 'faultcode', http_Booking['faultcode']));
                }
                else {
                    ...
                    resolve(ResponseBuilder_1.ResponseBuilder.successResponse('Data', hotelBookingDocument[0]));
                }
            }
            else {
    ...
    async http_Bookingcancel(body) {
        ...
        return new Promise(async (resolve) => {
            try {
                let xmlData = fs.readFileSync(path.resolve(__dirname, './soapXML/bookingCancel.xml'), 'utf-8');
                let _replace_data = xmlData.split('--TraceId--').join(body.TraceId);
    ...
    async bookingCancel(body) {
        return new Promise(async (resolve) => {
            ...
            if (BookingAsGuest == true) {
                ...
                if (httpStatus == 404) {
                    resolve(ResponseBuilder_1.ResponseBuilder.errorMultiResponse('message', http_CancelBooking['message'], 'faultcode', http_CancelBooking['faultcode']));
                }
                else {
                    ...
                    resolve(ResponseBuilder_1.ResponseBuilder.successResponse('Data', hotelCancelBookingDocument[0]));
                }
            }
            else {
                let accountFindData = await this.mongodb.findAccountUsersDocument({ _id: { $eq: new mongodb_2.ObjectId(body.UserID) }, "ActiveSession": { $eq: body.ActiveSession } });
    ...
    async userBookingDetails(body) {
        return new Promise(async (resolve) => {
            ...
            let hotelBookingDocument = await this.mongodb.findHotelBookingDocument({ _id: { $eq: new mongodb_2.ObjectId(bookingid) } });
            let accountFindDataLength = hotelBookingDocument.length;
            if (accountFindDataLength == 1) {
                resolve(ResponseBuilder_1.ResponseBuilder.successResponse('Data', hotelBookingDocument[0]));
            }
            else {
                resolve(ResponseBuilder_1.ResponseBuilder.errorResponse('message', "No Booking"));
            }
        });
    }
    async FilterBylistCities(client, data) {
...
__decorate([
    (0, common_1.Inject)(amazon_sdk_1.AmazonSDK),
    __metadata("design:type", amazon_sdk_1.AmazonSDK)
], HotelRepo.prototype, "amazon", void 0);
HotelRepo = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [mongodb_1.MongoDatabase])
], HotelRepo);
exports.HotelRepo = HotelRepo;
//# sourceMappingURL=hotel.repo.js.map